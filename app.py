"""
app.py — Single-file Flask School Website
==========================================
Requirements:
...
DB_PASSWORD = os.environ.get('DB_PASSWORD', 'Divyashi@07')
...
MAIL_USERNAME_VAL = os.environ.get('MAIL_USERNAME', '')   # ← your Gmail address
MAIL_PASSWORD_VAL = os.environ.get('MAIL_PASSWORD', '')   # ← Gmail App Password
MAIL_RECEIVER     = os.environ.get('MAIL_RECEIVER', '')   # ← where to receive contact msgs
...
for _sub in ('notices', 'events', 'gallery', 'downloads', 'blog', 'misc', 'board'):
...
class BoardMember(db.Model):
    ...
    id           = db.Column(db.Integer,     primary_key=True)
    name         = db.Column(db.String(150), nullable=False)
    position     = db.Column(db.String(150), nullable=False)          # e.g. Chairperson
    category     = db.Column(db.String(80),  default='board')         # board / management / faculty
...
class Topper(db.Model):
    ...
    id           = db.Column(db.Integer,     primary_key=True)
    name         = db.Column(db.String(150), nullable=False)
    stream       = db.Column(db.String(50),  nullable=False)   # science_bio | science_math | management | law | arts
    percentage   = db.Column(db.Numeric(5,2),nullable=True)    # e.g. 96.25
...
class AnnouncementPopupForm(FlaskForm):
    """Separate form for the Announcement Popup — all fields optional so
    validation never silently blocks a save."""
...
def index():
    ...
    # Toppers — up to 20 per stream, grouped by stream
    ...
    # Compute popup visibility — use bool() so we handle True, 1, or any truthy value
    # (MySQL JSON can return 1 instead of True depending on driver version)
    try:
...
def download_file(file_id):
    ...
    return send_from_directory(folder, dl.filename, as_attachment=True,
...
def notice_attachment(notice_id):
    ...
    return send_from_directory(folder, notice.attachment, as_attachment=True)
...
''')
                    ...
                    # Email not configured — show the link directly (dev mode)
        ...
        for f in files:
            if f and f.filename and allowed_image(f.filename):
                ...
                if filename:
                    ...
                    count += 1
        db.session.commit()
        # Always update cover_image after upload
        if count:
...
def homepage_popup():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            # Surface validation errors so the admin can see what went wrong
...
def seed_database():
    ...
    if not HomepageContent.query.filter_by(section='about').first():
        db.session.add(HomepageContent(
            ...
            content=('Founded in 1990, Greenwood Academy has grown into a centre of academic '
    ...
    if not HomepageContent.query.filter_by(section='principal').first():
        db.session.add(HomepageContent(
            ...
            subtitle='Dr. Sarah Johnson, Principal',
            content=('Education is not merely the acquisition of knowledge; it is the cultivation '
                     'of character, curiosity, and compassion. At Greenwood Academy, every child '
    ...
    if not Testimonial.query.first():
        db.session.add(Testimonial(
            ...
            content='Greenwood Academy has been incredible for my daughter. The teachers are dedicated and the emphasis on values makes it truly special.',
...
with app.app_context():
    try:
        ...
        db.session.remove()   # always release the startup session
